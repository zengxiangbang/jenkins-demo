             ...
             steps {
                 ...
                 container('kubectl') {
                     ...
                     withCredentials([kubeconfigContent(credentialsId: 'k8sCertAuth', variable: 'KUBECONFIG_CONTENT')]) {
                        sh '''echo "$KUBECONFIG_CONTENT" > kubeconfig &&export KUBECONFIG=./kubeconfig &&# cat kubeconfig && kubectl autoscale deployment ${envs}-${repo} --min=2 --max=10 -n ${envs} --cpu-percent=50'''
                    }
